...
func (tr *TicketRepo) GetByEventID(eventID uuid.UUID) (tickets []model.Ticket, err error) {
...
func (tr *TicketRepo) Update(ticket *model.Ticket, tx ...*sqlx.Tx) error {
	...
	if local {
		tr.Log.Info("Transaction created by repo: committing")
		return t.Commit()
	}
...
func (tr *TicketRepo) DeleteBySlug(slug string, tx ...*sqlx.Tx) error {
...
func (tr *TicketRepo) DeleteByTicketname(ticketname string, tx ...*sqlx.Tx) error {
...
func (tr *TicketRepo) PreBook(eventSlug, ticketType string, qty int, reservationID, userSlug string, tx ...*sqlx.Tx) (tickets []model.Ticket, err error) {
	// Create a local transaction if it is not passed as argument.
	t, local, err := tr.getTx(tx)
	if err != nil {
		return tickets, err
...
func (tr *TicketRepo) ConfirmReservation(eventSlug, reservationID, userSlug string, tx ...*sqlx.Tx) (tickets []model.Ticket, err error) {
	// Create a local transaction if it is not passed as argument.
	t, local, err := tr.getTx(tx)
	if err != nil {
		return tickets, err
	...
	st = `UPDATE tickets
					SET
						...
						updated_at = now()
					WHERE tickets.id IN (
						SELECT tickets.id FROM tickets INNER JOIN events ON tickets.event_id = events.id WHERE events.slug = '%s' AND (tickets.is_deleted IS NULL OR NOT tickets.is_deleted) AND (events.is_deleted IS NULL OR NOT events.is_deleted) AND (reserved_by_id = '%s') AND (bought_by_id IS NULL OR bought_by_id::text='00000000-0000-0000-0000-000000000000') AND (reservation_id = '%s') AND (status='reserved') AND (gateway_op_id IS NULL or gateway_op_id=''));`

	st = fmt.Sprintf(st, userID, userID, eventSlug, userID, reservationID)
	...
	// Select all updated
	st = `SELECT tickets.* FROM tickets WHERE (tickets.is_deleted IS NULL OR NOT tickets.is_deleted) AND (reservation_id = '%s') AND (reserved_by_id = '%s') AND (bought_by_id = '%s') AND status='paid' ORDER BY tickets.updated_at ASC;`

	st = fmt.Sprintf(st, reservationID, userID, userID)
...
func (tr *TicketRepo) newTx() (tx *sqlx.Tx, err error) {
	tx, err = tr.DB.Beginx()
	if err != nil {
		return tx, err
	}
...
func (tr *TicketRepo) getTx(txs []*sqlx.Tx) (tx *sqlx.Tx, local bool, err error) {
